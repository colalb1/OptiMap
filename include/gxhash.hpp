...
namespace gxhash
{
    ...
    inline uint64_t gxhash64(const void* data, size_t len, uint64_t seed = 0) noexcept
    {
        ...
        if (data == nullptr)
        {
            ...
            return detail::final_avalanche(
                    seed ^ (static_cast<uint64_t>(orig_len) * 0x9e3779b97f4a7c15ULL)
            );
        }
        ...
        if (has_aes)
        {
            ...
            __m128i acc = _mm_set_epi64x(
                    static_cast<long long>(seed ^ C1),
                    static_cast<long long>((~seed) ^ C2)
            );
        ...
        while (remaining >= 16)
        {
            ...
            state += a * MUL1;
            uint64_t m = detail::mix64(
                    a ^ (detail::rotl64(b, 23) + (state ^ (state >> 41))),
    ...
    gxhash128(const void* data, size_t len, uint64_t seed = 0) noexcept
    {
    ...
    template <typename T> struct GxHash
    {
    ...
    // Pointers
    template <typename T> struct GxHash<T*>
    {
    ...
    template <> struct GxHash<std::nullptr_t>
    {
    ...
    template <typename T, typename Deleter> struct GxHash<std::unique_ptr<T, Deleter>>
    {
    ...
    template <typename T> struct GxHash<std::shared_ptr<T>>
    {
    ...
    // Strings
    template <> struct GxHash<std::string>
    {
    ...
    template <> struct GxHash<std::string_view>
    {
    ...
    template <> struct GxHash<const char*>
    {
    ...
    // Pair and tuple
    template <typename T1, typename T2> struct GxHash<std::pair<T1, T2>>
    {
    ...
    template <typename... Types> struct GxHash<std::tuple<Types...>>
    {
    ...
    template <typename T, typename Alloc> struct GxHash<std::vector<T, Alloc>>
    {
    ...
    template <typename T, std::size_t N> struct GxHash<std::array<T, N>>
    {
    ...
    template <typename T> struct GxHash<std::optional<T>>
    {
    ...
    template <typename... Types> struct GxHash<std::variant<Types...>>
    {
        std::size_t operator()(const std::variant<Types...>& v) const noexcept
        {
            return std::visit(
                    [](auto&& value) {
                        return GxHash<std::decay_t<decltype(value)>>{}(value);
                    },
                    v
            );
        }
    ...
    // Other STL types
    template <> struct GxHash<std::thread::id>
    {
    ...
    template <size_t N> struct GxHash<std::bitset<N>>
    {
