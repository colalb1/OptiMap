...
static inline uint64_t _wymix(uint64_t A, uint64_t B) {
    ...
    // Note: This implementation is likely incorrect as it doesn't produce a 128-bit intermediate
    // result.
...
template <>
struct WyHash<std::nullptr_t> {
    size_t operator()(std::nullptr_t) const noexcept { return 0; }
};

template <typename T, typename Deleter>
struct WyHash<std::unique_ptr<T, Deleter>> {
...
template <typename T>
struct WyHash<std::shared_ptr<T>> {
...
struct WyHash<std::tuple<Types...>> {
    size_t operator()(const std::tuple<Types...>& t) const noexcept {
        size_t seed = 0;
        std::apply([&](const auto&... args) { (hash_combine(seed, args), ...); }, t);
        return seed;
    }
};

template <typename T, typename Alloc>
struct WyHash<std::vector<T, Alloc>> {
