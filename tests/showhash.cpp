...
template <typename T> void showHash(T val)
{
    auto sh = std::hash<T>{}(val);
    auto mh = Hash<T>{}(val);
    std::cerr << hex(val) << " -> " << hex(sh) << " " << hex(mh) << " "
              << std::bitset<sizeof(size_t) * 8>(mh) << std::endl;
}

template <typename T> void showHash(const char* title)
{
...
void showHashString(std::string const& val)
{
    ...
    auto showVal = '"' + val + '"';
    std::cerr << std::setfill(' ') << std::setw(14) << showVal << " -> " << hex(mh) << " "
              << std::bitset<sizeof(size_t) * 8>(mh) << std::endl;
}
